...
class FederatedEnvironment():
    def __init__(self, args, run_name, envs, agent_idx, agent, optimizer):
        ...
        self.writer = SummaryWriter(f"runs/{run_name}_agent_{agent_idx}")
...
def local_update(federated_env, step, args) -> None:
    # TRY NOT TO MODIFY: start the game

    for update in range(1, args.local_updates + 1):
        # Annealing the rate if instructed to do so.
        # TODO
        num_updates = step * args.local_updates + update
        if args.anneal_lr:
        ...
        federated_env.writer.add_scalar("charts/SPS", int(global_step / (time.time() - start_time)), global_step)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.n_agents) as executor:
        for update in range(1, num_updates + 1):
            ...
            for i in range(args.n_agents):
                futures.append(executor.submit(local_update, federated_envs[i], update, args))

            for future in futures:
                future.result()

            exchange_weights(agents)
